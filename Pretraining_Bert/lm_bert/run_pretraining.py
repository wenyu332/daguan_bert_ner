...
def main():
    ...
    if args.do_train:
        ...
        # model.load_state_dict(torch.load('./outputs/60000_pytorch_model.bin'))
        if args.fp16:
            model.half()
        model.to(device)
        if args.local_rank != -1:
            ...
            for e in trange(int(args.num_train_epochs), desc="Epoch"):
                ...
                for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration")):
                    ...
                    if (step + 1) % args.gradient_accumulation_steps == 0:
                        ...
                        optimizer.zero_grad()
                        global_step += 1
                    if nb_tr_steps > 0 and nb_tr_steps % 10000 == 0:
                        logger.info("===================== -epoch %d -train_step %d -iter_nums %d -train_loss %.4f\n" % (e, nb_tr_steps, iter_nums,tr_loss / nb_tr_steps))

                    if nb_tr_steps > 0 and iter_nums % 50000 == 0:
                        ...
                        output_model_file = os.path.join(args.output_dir, str(eval_loss)[:4] + '_' + str(iter_nums) +'_iter_nums')
                        torch.save(model_to_save.state_dict(), output_model_file)
                        best_loss = eval_loss
                        logger.info("============================ -iter_nums %d -train_loss %.4f -eval_loss %.4f\n"% (iter_nums, tr_loss / nb_tr_steps, eval_loss))
    else:
        model = BertForMaskedLM(config=BertConfig.from_json_file(args.bert_config_json))
        model.load_state_dict(torch.load('./outputs/60000_pytorch_model.bin'))
